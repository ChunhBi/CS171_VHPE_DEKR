...
class PoseHigherResolutionNet(nn.Module):
    def __init__(self, cfg, **kwargs):
        ...
        self.encoder=TemporalEncoder(input_size=2*128*128,output_size=2*128*128)
    ...
    def forward(self, x):
        ...
        for j in range(self.num_joints):
            feature_offset=self.offset_feature_layers[j](
                        offset_feature[:,j*self.offset_prekpt:(j+1)*self.offset_prekpt])
            tmp_offset=self.offset_final_layer[j](feature_offset)
            offset_shape=tmp_offset.shape
            tmp_offset=self.encoder(tmp_offset.view(1,offset_shape[0],-1))
            tmp_offset=tmp_offset.view(offset_shape)
            final_offset.append(tmp_offset)
    ...
    def init_weights(self, pretrained='', verbose=True):
        ...
        for m in self.modules():
            if isinstance(m, nn.Conv2d):
                ...
                nn.init.constant_(m.bias, 0)
            elif isinstance(m,TemporalEncoder):
                m.init_weights()
